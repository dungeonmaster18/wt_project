...
public class HospitalServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            RegisterHospitalDAO rdao = new RegisterHospitalDAO();
            HospitalRF h = new HospitalRF(name, address, zipcode, phone_number, emailid, uname, password, hcode, date, htype);
            int c = rdao.createHospitalAccount(h);
            SendEmail s = new SendEmail();
            String message = "Hi " + name + ",You have been Successfully registered to our system";

            if (c > 0) {
                s.sendEmail(emailid,message,"For Verifiaction");
                RequestDispatcher view = request.getRequestDispatcher("welcome.jsp");
                view.forward(request, response);
            } else {
                response.sendRedirect("formh.jsp");
            }
        }
    }
